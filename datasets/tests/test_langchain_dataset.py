...
def test_document_storage():
    ...
    assert len(
        dataset.add_texts(sample_text, sample_metas, skip_invalid=True)) == 1  # only one text should be added
    ...
    assert dataset.get_by_id(stored_docs[0][0], include=["metadatas"])["metadatas"], \
        "get_by_id failed"

    print("Found not empty", dataset.get(where={"source": "sth"}, include=['documents']))

    assert dataset.get_containing_field("file_path",  include=["metadatas"])["metadatas"], \
        "file_path should be in metadata for pdf files"

    # Searching
    assert dataset.search_by_name('2302.00386.pdf', regex_match=True, include=["metadatas"])["metadatas"],\
        "search_by_name should contain found data"

    assert len(
        dataset.similarity_search("new feature", n_results=2)) == 2, "similarity_search should return 2 results"


@pytest.mark.skipif(not OPENAI_AVAILABLE, reason="OpenAI not available (not installed or invalid key)")
def test_llm():
  ...
  {format_instructions}
  """
    create_and_register_prompt(name="identify_features", template=text, input_variables=["text", "format_instructions"])

    prompt = PromptTemplate(template=text, input_variables=["text", "format_instructions"])
